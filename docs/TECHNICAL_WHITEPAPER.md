...
> **⚠️ 注意（2026-03-29追記）**: 本文書はSpring Bootバックエンドを含むアーキテクチャを記述していますが、Spring Bootバックエンドは設計のみで実装されていません。現在のPDF/画像生成はクライアントサイド（html2canvas + jsPDF）で完結しています。また、質問数は17問から21問に変更されています。本文書は設計検討時の記録として保持しますが、現在のシステム構成とは異なる箇所があります。
